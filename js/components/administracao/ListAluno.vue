...
<table class="table table-striped table-hover">
      ...
      <tbody>
        <tr v-for="(item,index) in lista">
          <td>{{item.id}}</td>
           <td><img :src="'http://producao.yetoafrica.com/nossafrica/storage/app/public/'+item.foto" alt="" class="img-responsive img-rounded img-thumbnail" width="120px"></td> 
          <td>{{item.name}}</td>
          <td>{{item.email}}</td>
          <td>{{item.valor}}</td>
...
<script>
    export default {
      props:['titulos','itens','ordem','ordemcol','criar','totalC','totalD'],
      data: function(){
        return {
          buscar:'',
